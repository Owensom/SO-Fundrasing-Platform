...
export default function AdminRaffleEditPage() {
      ...
      try {
        ...
        const config =
          raffle && typeof (raffle as any).config_json === "object" && (raffle as any).config_json
            ? ((raffle as any).config_json as Record<string, any>)
            : {};
        ...
        setImageUrl((raffle as any).imageUrl ?? (raffle as any).image_url ?? "");

        setStartNumber(
          config.startNumber !== undefined && config.startNumber !== null
            ? String(config.startNumber)
            : (raffle as any).startNumber !== undefined
              ? String((raffle as any).startNumber)
              : "",
        );

        setEndNumber(
          config.endNumber !== undefined && config.endNumber !== null
            ? String(config.endNumber)
            : (raffle as any).endNumber !== undefined
              ? String((raffle as any).endNumber)
              : "",
        );

        setTicketPrice(
          (raffle as any).ticketPrice !== undefined
            ? String((raffle as any).ticketPrice)
            : (raffle as any).ticket_price !== undefined
              ? String((raffle as any).ticket_price)
              : "",
        );

        setCurrency((((raffle as any).currency as CurrencyCode) || "GBP"));

        const configOffers = Array.isArray(config.offers) ? config.offers : raffle.offers ?? [];
        setOffers(
          configOffers.map((offer: any, index: number) => ({
            id:
            ...
            quantity:
              offer?.quantity !== undefined
                ...
                : offer?.tickets !== undefined
                  ? String(offer.tickets)
                  : "",
            price: offer?.price !== undefined ? String(offer.price) : "",
          })),
        );

        setColours(Array.isArray(config.colours) ? config.colours : []);
      } catch (error) {
        if (cancelled) return;
        setExistingRaffleId("");
        setStatusMessage(
  ...
  const validOffers = useMemo(() => {
    return offers
      ...
      .map((offer, index) => ({ ...offer, sort_order: index }))
  ...
  async function persist() {
    ...
    if (existingRaffleId && existingRaffleId !== "create") {
      const updated = await updateRaffle(existingRaffleId, payload as any);
      const nextId = String(updated.id ?? existingRaffleId);
      setExistingRaffleId(nextId);
      return updated;
    }

    if (routeId && routeId !== "create" && routeId !== slug) {
      const updated = await updateRaffle(routeId, payload as any);
      const nextId = String(updated.id ?? routeId);
      setExistingRaffleId(nextId);
      return updated;
    }

    const created = await createRaffle(payload as any);
  ...
  async function handleSaveDraft(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const nextSlugOrId = saved.slug || saved.id;
      if (nextSlugOrId) {
        const nextRoute = `/admin/raffles/${nextSlugOrId}`;
        if (router.asPath !== nextRoute) {
    ...
    <div style={pageStyle}>
      <form onSubmit={handleSaveDraft}>
        ...
        <div style={statsGridStyle}>
          ...
          <StatCard
            label="Max revenue"
            value={expectedRevenue > 0 ? formatCurrency(expectedRevenue, currency) : "—"}
          />
        </div>
          ...
          <div style={leftColumnStyle}>
            ...
            >
              <div style={fourColGridStyle}>
                ...
                <Field label={`Single ticket price (${currencySymbol(currency)})`}>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
                    ...
                    return (
                      <div key={offer.id} style={offerCardStyle}>
                        ...
                        <div style={offerMetaStyle}>
                          <span>
                            {pricePerTicket !== null
                              ? `${formatCurrency(pricePerTicket, currency)} per ticket`
                              : "Incomplete offer"}
                          </span>
                          <span>
                            {parsedTicketPrice !== null
                              ? `vs base ${formatCurrency(parsedTicketPrice, currency)} each`
                              : "Base price not set"}
                          </span>
                        </div>
                      </div>
                ...
                <div style={previewBodyStyle}>
                  ...
                  <div style={previewMetaGridStyle}>
                    ...
                    <PreviewStat
                      ...
                      value={
                        parsedTicketPrice !== null
                          ? formatCurrency(parsedTicketPrice, currency)
                          : "—"
                      }
                    />
                  </div>
            ...
            <SectionCard title="Pricing insight" subtitle="Quick commercial summary.">
              <div style={{ display: "grid", gap: 12 }}>
                ...
                <InsightRow
                  ...
                  value={
                    bestOffer
                      ? `${bestOffer.label} (${formatCurrency(
...
const fourColGridStyle: React.CSSProperties = {
  display: "grid",
  gridTemplateColumns: "repeat(4, minmax(0, 1fr))",
  gap: 16,
};
