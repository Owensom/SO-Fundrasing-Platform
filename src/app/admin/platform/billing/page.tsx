...
type TenantBillingRow = {
  ...
  published_raffles: number | string | null;
  published_squares: number | string | null;
  published_events: number | string | null;
  published_auctions: number | string | null;
};
...
type TenantBillingFormState = {
  ...
  published_raffles: number;
  published_squares: number;
  published_events: number;
  published_auctions: number;
};
...
async function requirePlatformOwner() {
  ...
  if (!(session.user as any).isPlatformOwner) {
...
function normaliseTenantBilling(row: TenantBillingRow): TenantBillingFormState {
  ...
  return {
    tenant_slug: cleanText(row.tenant_slug),
    tenant_name: cleanText(row.tenant_name, cleanText(row.tenant_slug)),
    ...
    stripe_subscription_id: cleanText(row.stripe_subscription_id),
    stripe_connect_account_id: cleanText(row.stripe_connect_account_id),
    subscription_status: cleanText(row.subscription_status, "active"),
    buyer_fee_contributions_enabled: Boolean(
    ...
    published_raffles: safeCount(row.published_raffles),
    published_squares: safeCount(row.published_squares),
    published_events: safeCount(row.published_events),
  ...
  const rows = await query<TenantBillingRow>(
    `
      select
        ...
        coalesce(raffle_counts.published_count, 0)::int as published_raffles,
        coalesce(square_counts.published_count, 0)::int as published_squares,
        coalesce(event_counts.published_count, 0)::int as published_events,
        coalesce(auction_counts.published_count, 0)::int as published_auctions
      from tenants t
      left join tenant_settings ts
        on ts.tenant_slug = t.slug
      left join (
        select tenant_slug, count(*)::int as published_count
        from raffles
        where status = 'published'
        group by tenant_slug
      ) raffle_counts
        on raffle_counts.tenant_slug = t.slug
      left join (
        select tenant_slug, count(*)::int as published_count
        from squares_games
        where status = 'published'
        ...
        on square_counts.tenant_slug = t.slug
      left join (
        select tenant_slug, count(*)::int as published_count
        from events
        where status = 'published'
        ...
        on event_counts.tenant_slug = t.slug
      left join (
        select tenant_slug, count(*)::int as published_count
        ...
        on auction_counts.tenant_slug = t.slug
      order by t.slug asc
    `,
  );
          ...
          <div className="tenantGrid" style={styles.tenantGrid}>
            {tenants.map((tenant) => {
              const publishedCampaigns =
                tenant.published_raffles +
                tenant.published_squares +
                tenant.published_events;
                ...
                >
                  ...
                  <div className="tenantCardHeader" style={styles.tenantCardHeader}>
                    <div>
                      ...
                      <div style={styles.tenantName}>{tenant.tenant_name}</div>
