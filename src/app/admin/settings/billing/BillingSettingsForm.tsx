...
export default function BillingSettingsForm({
  ...
  const [capabilityValues, setCapabilityValues] = useState({
    ...
    crm_enabled: formState.crm_enabled,
    auctions_enabled: formState.auctions_enabled,
    reserved_seating_enabled: formState.reserved_seating_enabled,
    finance_dashboard_enabled: formState.finance_dashboard_enabled,
    white_label_enabled: formState.white_label_enabled,
    custom_domain_enabled: formState.custom_domain_enabled,
  });
    ...
    () => [
      {
        ...
        enabled: capabilityValues.crm_enabled || tier !== "community",
      },
      {
        label: "Auctions",
        key: "auctions_enabled" as const,
        enabled: capabilityValues.auctions_enabled || tier !== "community",
      },
      {
        label: "Reserved seating",
        key: "reserved_seating_enabled" as const,
        enabled:
          capabilityValues.reserved_seating_enabled || tier !== "community",
      },
      {
        label: "Finance dashboard",
        key: "finance_dashboard_enabled" as const,
        enabled:
          capabilityValues.finance_dashboard_enabled || tier !== "community",
      },
      {
        label: "White label",
        key: "white_label_enabled" as const,
        enabled: capabilityValues.white_label_enabled || tier === "foundation",
      },
      {
        label: "Custom domain",
        key: "custom_domain_enabled" as const,
        enabled: capabilityValues.custom_domain_enabled || tier === "foundation",
      },
    ],
    [capabilityValues, tier],
  );

  function handleTierChange(value: TierKey) {
    setTier(value);
    setPlatformFeePercent(defaultFeeForTier(value));
  }
          ...
          <div className="toggleGrid" style={styles.toggleGrid}>
            <label style={styles.toggleCard}>
              <input
                ...
                checked={capabilityValues.buyer_fee_contributions_enabled}
                onChange={(event) =>
                  setCapabilityValues((current) => ({
                    ...current,
                    buyer_fee_contributions_enabled: event.target.checked,
                  }))
                }
              />
        ...
        <section style={styles.sideColumn}>
          <article style={styles.sideCard}>
            ...
            <div style={styles.tiersList}>
              {(Object.entries(TIER_DETAILS) as [TierKey, typeof currentTier][]).map(
                ([key, value]) => (
                  <div
                    key={key}
                    style={
                      key === tier ? styles.activeTierCard : styles.tierCard
                    }
                  >
...
.billing-page, .billing-page * { box-sizing: border-box; }
.billing-page { overflow-x: hidden; }
.billing-page section, .billing-page article, .billing-page div, .billing-page form, .billing-page label { min-width: 0; }

@media (max-width: 980px) {
  .billing-page .hero, .billing-page .contentGrid { grid-template-columns: 1fr !important; }
  .billing-page .heroSummaryGrid, .billing-page .formGrid, .billing-page .toggleGrid { grid-template-columns: repeat(2, minmax(0, 1fr)) !important; }
}

@media (max-width: 680px) {
  .billing-page { padding: 16px 10px 44px !important; }
  .billing-page .hero, .billing-page .formCard, .billing-page .sideCard { padding: 18px !important; border-radius: 24px !important; }
  .billing-page .title { font-size: clamp(36px, 12vw, 52px) !important; line-height: 0.98 !important; }
  .billing-page .heroActions, .billing-page .heroSummaryGrid, .billing-page .formGrid, .billing-page .toggleGrid, .billing-page .submitRow, .billing-page .statusRow, .billing-page .cardHeader, .billing-page .tierTop { grid-template-columns: 1fr !important; }
  .billing-page .heroButton, .billing-page .heroButtonLight, .billing-page .saveButton { width: 100% !important; justify-content: center !important; text-align: center !important; }
}
`;
