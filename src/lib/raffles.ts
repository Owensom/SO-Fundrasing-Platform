...
function normaliseImagePosition(value: unknown): string {
...
async function getCurrentConfig(
  ...
  return {
    ...
    image_position: normaliseImagePosition(raw.image_position),
  };
}
...
export async function updateRaffleOffers(
  ...
  const clean = offers.map((offer, index) => ({
...
export async function updateRaffleColours(
  ...
  const clean = colours.map((colour, index) => ({
...
export async function updateRaffleImagePosition(
  ...
  const config = await getCurrentConfig(id, tenantSlug);
  const imagePosition = normaliseImagePosition(position);
...
export async function updateRafflePrizes(
  ...
  const clean = prizes.map((prize, index) => ({
