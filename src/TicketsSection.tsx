...
export default function TicketsSection() {
  ...
  const [selectedSeatIdsByEvent, setSelectedSeatIdsByEvent] = useState<Record<string, string[]>>({});
  const [selectedTableByEvent, setSelectedTableByEvent] = useState<Record<string, string>>({});
  const [quantityByEvent, setQuantityByEvent] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    async function loadEvents() {
      ...
      try {
        ...
        setDrafts(
          loadedEvents.reduce((acc: EventDrafts, event: TicketEvent) => {
            acc[event.id] = {
        ...
        setTableDrafts(
          loadedEvents.reduce((acc: TableDrafts, event: TicketEvent) => {
            acc[event.id] = event.tables.reduce((tableAcc, table) => {
              tableAcc[table.id] = {
                name: table.name,
                seats: String(table.seats),
              };
              return tableAcc;
            }, {} as Record<string, { name: string; seats: string }>);
            return acc;
          }, {})
        );

        setSelectedSeatIdsByEvent(
          loadedEvents.reduce((acc: Record<string, string[]>, event: TicketEvent) => {
            acc[event.id] = [];
            return acc;
          }, {})
        );

        setSelectedTableByEvent(
          loadedEvents.reduce((acc: Record<string, string>, event: TicketEvent) => {
            acc[event.id] = event.tables[0]?.id ?? "";
            return acc;
          }, {})
        );
        ...
        if (loadedEvents.length > 0) {
          setActiveEventId((curr) =>
            curr && loadedEvents.some((e: TicketEvent) => e.id === curr) ? curr : loadedEvents[0].id
          );
        } else {
          setActiveEventId("");
        }
      } catch (err) {
        setAdminMessage(err instanceof Error ? err.message : "Unable to load tickets.");
      } finally {
        setEventsLoading(false);
      }
    }
      ...
      <div style={pageStyle()}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <section style={cardStyle()}>
            ...
            <p>{isLoggedIn ? "No ticket events found for this tenant yet." : "Please log in."}</p>
          </section>
        </div>
      </div>
    );
  ...
  const draft = event
    ...
    : { price: "0", rows: "0", seatsPerRow: "0" };

  const selectedSeats = event ? selectedSeatIdsByEvent[event.id] ?? [] : [];
  const selectedTableId = event ? selectedTableByEvent[event.id] ?? event.tables[0]?.id ?? "" : "";
  const selectedTable = event?.tables.find((t) => t.id === selectedTableId);
  const quantity = event ? Math.max(0, Number(quantityByEvent[event.id] || "0") || 0) : 0;

  const rowsCountInvalid =
    !!event && event.mode === "rows" && (draft.rows.trim() === "" || Number(draft.rows) <= 0);
  ...
  const totalRowSeats = event ? event.rows * event.seatsPerRow : 0;
  const availableRowSeats = event ? totalRowSeats - event.soldSeatIds.length : 0;
  const totalTableSeats = event ? event.tables.reduce((sum, t) => sum + t.seats, 0) : 0;
  const totalSoldTables = event ? event.tables.reduce((sum, t) => sum + t.sold, 0) : 0;
  const availableSelectedTable = selectedTable
    ? Math.max(selectedTable.seats - selectedTable.sold, 0)
    : 0;

  const total =
    event?.mode === "rows" ? selectedSeats.length * event.price : quantity * (event?.price ?? 0);
  ...
  function setEventPatch(id: string, patch: Partial<TicketEvent>) {
    setEvents((curr) => curr.map((e) => (e.id === id ? { ...e, ...patch } : e)));
  }

  function setDraftPatch(id: string, patch: Partial<EventDrafts[string]>) {
    setDrafts((curr) => ({
      ...curr,
  ...
  function setTableDraftPatch(
    eventId: string,
    tableId: string,
    patch: Partial<TableDrafts[string][string]>
  ) {
    setTableDrafts((curr) => ({
      ...curr,
      [eventId]: {
  ...
  async function addEvent() {
    ...
    try {
      ...
      setSelectedTableByEvent((curr) => ({ ...curr, [newEvent.id]: "" }));
  ...
  async function removeCurrentEvent() {
    ...
    try {
      await apiFetch(`/api/tickets/${event.id}`, {
        method: "DELETE",
      });
  ...
  async function saveCurrentEvent() {
    ...
    try {
      const saved = await apiFetch(`/api/tickets/${event.id}`, {
        method: "PUT",
        body: JSON.stringify({
          title: event.title,
  ...
  function buildReceipt(purchase?: TicketPurchase) {
    ...
    doc.setFontSize(12);
    doc.text(`Event: ${purchase?.eventTitle ?? event.title}`, 20, 38);
    doc.text(`Buyer: ${purchase?.buyerName ?? buyerName || "Not entered"}`, 20, 48);
    doc.text(`Email: ${purchase?.buyerEmail ?? buyerEmail || "Not entered"}`, 20, 58);
    doc.text(`Mode: ${purchase?.mode ?? event.mode}`, 20, 68);

    if ((purchase?.mode ?? event.mode) === "rows") {
      const receiptSeats = purchase?.seats ?? selectedSeats;
      doc.text(`Selected seats: ${receiptSeats.length ? receiptSeats.join(", ") : "None"}`, 20, 78);
      doc.text(`Quantity: ${purchase?.quantity ?? receiptSeats.length}`, 20, 88);
    } else {
      doc.text(`Table: ${purchase?.tableName ?? selectedTable?.name ?? "None"}`, 20, 78);
      doc.text(`Quantity: ${purchase?.quantity ?? quantity}`, 20, 88);
    }

    doc.text(`Price each: ${money(event.price)}`, 20, 98);
    doc.text(`Total: ${money(purchase?.total ?? total)}`, 20, 108);
    return doc;
  }
  ...
  async function buyTickets() {
    ...
    try {
      ...
      if (event.mode === "rows") {
        response = await apiFetch("/api/tickets/purchase", {
          method: "POST",
          body: JSON.stringify({
            eventId: event.id,
      ...
      const purchase: TicketPurchase = {
        ...
        mode: purchaseFromServer.details?.mode ?? event.mode,
        seats: purchaseFromServer.details?.seats ?? [],
        tableName: purchaseFromServer.details?.tableName,
        quantity: purchaseFromServer.quantity,
        total: purchaseFromServer.total,
        createdAt: purchaseFromServer.createdAt,
      };

      if (purchase.mode === "rows") {
        appendLedger({
          ...
          description: `Seats: ${purchase.seats.join(", ")}`,
        ...
        appendLedger({
          ...
          buyerName: purchase.buyerName,
          buyerEmail: purchase.buyerEmail,
          description: `Table: ${purchase.tableName ?? "Unknown"} × ${purchase.quantity}`,
          quantity: purchase.quantity,
          total: purchase.total,
          createdAt: purchase.createdAt,
        });
      ...
      setSelectedSeatIdsByEvent((curr) =>
        purchase.mode === "rows" ? { ...curr, [event.id]: [] } : curr
      );

      if (purchase.mode === "tables") {
        setQuantityByEvent((curr) => ({ ...curr, [event.id]: "0" }));
      }
      ...
      doc.save(`${purchase.eventTitle.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-receipt.pdf`);
    } catch (err) {
